...
-- as(alias) 별칭
-- sql 문(쿼리문) 의 컬럼 또는 테이블이 별칭을 달 수 있다.
...
-- inner join
-- 두테이블의 교집합을 반환하는 join의 유형
-- inner join 에서 inner는 생각이 가능하다.

-- on 키워드를 사용한 join 방법
-- 컬럼명이 같거나, 다를 경우 on으로 서로 연관있는 컬럼에 대한 조건 작성

-- 메뉴의 이름과 카테고리의 이름을 조회
select
	a.menu_name,		-- 메뉴 이름 조회
    b.category_name		-- 카테고리 이름 조회
from
	tbl_menu a			-- 아무 테이블
    join
    tbl_category b		-- 2번째 테이블
    on 
    a.category_code = b.category_code;		-- 같은 카테고리 코드가 있음
...
select
    ...
    b.category_name
from
	tbl_menu a cross join tbl_category b;	-- 가능한 모든 조합을 조회
...
select
    ...
    b.category_name
from
	tbl_category a join tbl_category bfk_category_code  -- 공유하고 있는 컬럼값이 다르기 때문에 using 사용 못함fk_category_code
    on
    a.ref_category_cofk_category_codede = b.category_code;
...
-- 3. 어떤 테이블을 기준으로 join 을 수행할 것인지 (inner, left, right, cross, self) 선택
