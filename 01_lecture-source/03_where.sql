...
from
	tbl_menu
where
	orderable_status = 'Y';		-- orderable_status가 Y와 일치하는 컬럼들만 조회
...
-- 메뉴 테이블에서 모든 컬럼조회
-- 가격이 10000원 이상 20000원 미만인 메뉴를 조회
...
-- 메뉴 테이블에서 모든 컬럼조회
-- 단, 판매상태가 Y 이거나 카테고리 코드가 10인 메뉴조회
...
-- 메뉴 테이블에서 모든 컬럼조회
-- 단, 판매상태가 Y 이면서 카테고리 코드가 10인 메뉴조회
-- 그리고 카테고리 코드로 오름차순 정렬
...
-- category_code = 4 or category_code = 5 or category_code = 6  <- 이 식이 축약됨
