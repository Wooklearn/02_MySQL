...
-- 만들어진 view 조회		-- 데이터베이스가 할 일을 줄여줌 21개 검색할걸 4번으로 줄임
select * from hansik; 

describe hansik;

-- 한식이라고 하는 view 는 원본인 tbl+menu 에서 만든 가상의 테이블
insert into tbl_menu value (null, '해장국', 5500, 4, 'Y');
...
-- view 사용 시 dml 주의점
-- 1. 뷰 정의할 때 포함되지 않는 컬럼을 사용하려고 하는 경우
